"""
Script that is called from main.py to evaluate TTA+Ens method for predictive uncertainty on the test set, 
and obtain conformal quantiles on calibration data.

It does so by running TTA within a loop for each ensemble member and aggregating.
"""
...
def eval_test(model: torch.nn.Module,
    ...
    for city in cities:
        ...
        pred = get_predictive_uncertainty(ens).to(device)  # aggregate both uncertainties
        del ens
