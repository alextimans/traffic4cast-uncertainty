"""
This script contains all the generated plots for the unsupervised outlier detection application.
...
def save_fig_subdir(fig_path: str, city: str, uq_method: str, subdir: str, filename: str):
    file_path = Path(os.path.join(fig_path, city, subdir))
    file_path.mkdir(exist_ok=True, parents=True)
    plt.savefig(os.path.join(file_path, f"{uq_method}_{filename}.png"),
                dpi=300, format="png", bbox_inches='tight')
    print(f"Saved figure to {file_path}.")
...
# out_morning/evening for unet, out_morning/evening_pp for unet++
fig_path = "./figures/out_morning_pp"
base_path = "./results/out_morning_pp"
...
all_cities = ["BANGKOK", "BARCELONA", "MOSCOW"]

# tuple (r, c, m)
crops_by_city = {"BANGKOK": [(285,240,20), (295,190,30), (328,200,20)],
...
# tuple (r_m, c_m, num_m, r, c, m), used only for last two plots
out_crops_by_city = {"BANGKOK": [],
                     "BARCELONA": [],
                      "MOSCOW": [
                          (150,60,30,152,60,8),
                          (170,250,30,174,247,11),
                          (358,227,30,358,226,10),
                          (217,305,30,219,305,16),
                          (270,93,30,271,93,12)
                          ]
                      }

pixels_by_city = {"BANGKOK": [(231,428), (311,266), (463,24)],
                  "BARCELONA": [(213,302), (386,192), (470,47)],
                  "MOSCOW": [(16,206), (100,265), (245,405)]}
# morning
samp_by_city = {"BANGKOK": [24, 66, 81],
                  "BARCELONA": [24, 45, 89],
                  "MOSCOW": [26, 45, 89]}
# evening
# samp_by_city = {"BANGKOK": [25, 45, 76],
#                   "BARCELONA": [30, 45, 89],
#                   "MOSCOW": [12, 45, 89]}

# =============================================================================
# RUN ALL PLOTS FOR ALL CITIES
# =============================================================================
...
for city in all_cities:
    ...
    r=219; c=305; num=16 # MODIFY HERE
    r=271; c=93; num=12 # MODIFY HERE
    ...
    # =============================================================================
    from scipy.stats import combine_pvalues # Fisher's method

    pval = load_h5_file(os.path.join(base_path, city, f"pval_{uq_method}.h5"))
    out = load_h5_file(os.path.join(base_path, city, f"out_{ob}_{uq_method}.h5"))

    # find interesting pixel
    # (out[..., 2].sum(dim=0) == 71).nonzero(as_tuple=False)

    pixels = pixels_by_city[city] # CALLS PIXELS
    for pix_h, pix_w in pixels:
    ...
    for pix_h, pix_w in pixels:
        ...
        lab = ["Volume", "Speed"]
        for i, ax in enumerate(axes.flat):
            ax.hist(pred_pix[:, i].numpy(), bins=15, alpha=0.6,
                    color="red", label="Test set", density=True)
            ax.hist(pred_tr_pix[:, i].numpy(), bins=15, alpha=0.6,
    ...
    labels = ["Ground truth", "Prediction", "Uncertainty"]
    crops = crops_by_city[city] # CALLS CROPS
    for r, c, num in crops:

        for i, d in enumerate(list(val.keys())):
            label = labels[i]
            ...
            save_fig_subdir(fig_path, city, uq_method,
                            f"crop_{r}_{c}", f"val_{d}_{ob}_crop_{r}_{c}")
            plt.close()
    ...
    for r, c, num in crops:
        ...
        for i, d in enumerate(list(val.keys())):
            ...
            save_fig_subdir(fig_path, city, uq_method,
                            f"crop_{r}_{c}", f"val_{d}_train_{ob}_crop_{r}_{c}")
            plt.close()
    ...
    labels = ["Ground truth", "Prediction", "Uncertainty"]
    crops = out_crops_by_city[city] # CALLS CROPS
    for r_m, c_m, num_m, r, c, num in crops:
        ...
        for i, d in enumerate(list(val.keys())):
            ...
            save_fig_subdir(fig_path, city, uq_method,
                            f"crop_out_test_{r}_{c}", f"val_{d}_{ob}_crop_{r_m}_{c_m}_OUT_{r}_{c}")
            plt.close()
    ...
    for r_m, c_m, num_m, r, c, num in crops:
        ...
        for i, d in enumerate(list(val.keys())):
            ...
            save_fig_subdir(fig_path, city, uq_method,
                            f"crop_out_train_{r}_{c}", f"val_{d}_train_{ob}_crop_{r_m}_{c_m}_OUT_{r}_{c}")
            plt.close()
